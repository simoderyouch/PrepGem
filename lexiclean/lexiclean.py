    ...
    if isinstance(data, str):
        ...
        for column in columns:
            df[column] = df[column].apply(
                lambda x: ' '.join([str(TextBlob(word).correct()) for word in nltk.word_tokenize(x.lower())]))
        return df
    else:
        raise ValueError("Invalid input. Please provide either a single text or a DataFrame with column names.")
...
"""
    ...
    if isinstance(data, str):
        corrected_words = spell_corrector(data)
        cleaned_text = remove_nonsense_words(corrected_words)
        return cleaned_text
    elif isinstance(data, pd.DataFrame) and columns:
        df = data.copy()
        for column in columns:
            df[column] = df[column].apply(lambda x: remove_nonsense_words(spell_corrector(x)))
        return df
    else:
        raise ValueError("Invalid input. Please provide either a single text or a DataFrame with column names.")
...
def _preprocess_single_text(text, params=None, join=False):
    ...
    if params is None:
        params = preprocessing_pipline.copy()
    else:
    ...
    for key, value in params.items():
        ...
        if value:
            if key == "clean_html_text":
                preprocessed_text = clean_html_text(preprocessed_text)
                ...
                preprocessed_text = remove_emojis(preprocessed_text)
            elif key == "remove_foreign_letters":
                preprocessed_text = remove_foreign_letters(preprocessed_text)
            elif key == "remove_numbers":
                preprocessed_text = remove_numbers(preprocessed_text)
            elif key == "lowercasing":
                preprocessed_text = lowercasing(preprocessed_text)
            elif key == "remove_white_spaces":
                preprocessed_text = remove_white_spaces(preprocessed_text)
            elif key == "remove_repeated_characters":
                preprocessed_text = remove_repeated_characters(preprocessed_text)
            elif key == "spell_corrector":
                preprocessed_text = spell_corrector(preprocessed_text)
            elif key == "remove_nonsense_words":
                preprocessed_text = remove_nonsense_words(preprocessed_text)
            elif key == "nosense_words_and_spell_check":
                preprocessed_text = nosense_words_and_spell_check(preprocessed_text)
            elif key == "tokenize":
